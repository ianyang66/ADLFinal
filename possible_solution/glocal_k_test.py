...
def load_data_100k(path='./', delimiter='\t'):
    ...
    test = np.loadtxt(path+'hahow-100k-test.txt', skiprows=0, delimiter=delimiter).astype('int32')
    ...
    test_r = np.zeros((n_u), dtype='float32')
...
path = data_path + './possible_solution/'
n_u = load_data_100k(path=path, delimiter='\t')
...
ckpt = torch.load('./possible_solution/ckpt/hahow-100k.pt', map_location=torch.device(device))
"""## Fine-tuning"""

complete_model = CompleteNet(model, n_u, n_u, n_hid, n_dim, n_layers, lambda_s, lambda_2, gk_size, dot_scale).double().to(device)
